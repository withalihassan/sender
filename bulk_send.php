...
include('db.php'); // This file must initialize the $pdo connection
...
<body>
  <div class="container">
    ...
    <form id="bulk-otp-form">
      ...
      <select id="region" name="region" required>
        ...
        <option value="ap-south-2">Hyedrabad (ap-south-2)</option>
      </select>
  ...
  <script>
    $(document).ready(function() {
      ...
      // When region is selected, fetch allowed numbers from ajax_handler.php
      ...
      $('#start-bulk-otp').click(function() {
        if (allowedNumbers.length === 0) {
          alert("No allowed numbers available. Please select a region first.");
          return;
        }
        ...
        // Add one OTP task for each of the next 9 numbers (if available)
        ...
        // Reset the total success counter before starting
        totalSuccess = 0;
      ...
      function processNextTask(tasks, index, awsKey, awsSecret) {
        ...
        $.ajax({
          ...
          data: {
            ...
            awsKey: awsKey,
            awsSecret: awsSecret,
            session_id: $('#session_id').val() // Pass session_id
          },
          success: function(response) {
            if (response.status === 'success') {
              addSentNumberRow(task.id, task.phone, 'OTP Sent');
              totalSuccess++; // Increment on successful send
              var secondsLeft = 5;
              ...
              // If monthly spend limit reached, mark as skipped and continue immediately
              addSentNumberRow(task.id, task.phone, 'Skipped: Monthly Spend Limit Reached');
              processNextTask(tasks, index + 1, awsKey, awsSecret);
            } else {
              // For any other error, show the error and halt further processing.
      ...
      $(document).on('click', '.update-status', function() {
        ...
        $.ajax({
          ...
          data: { 
            ...
            session_id: $('#session_id').val() // Pass session_id if needed
          },
          success: function(response) {
            if (response.status === 'success') {
