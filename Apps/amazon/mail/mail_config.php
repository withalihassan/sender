...
function load_cli_env_from_htaccess()
{
    ...
    foreach (file($path, FILE_IGNORE_NEW_LINES) as $line) {
        if (preg_match('/^SetEnv\s+(SMTPDEV_TOKEN|SMTPDEV_BASE|MOCK_VERIFY_ALLOWED_HOSTS|MOCK_VERIFY_BUTTON_SELECTOR|MOCK_VERIFY_SECOND_BUTTON_SELECTOR)\s+(.+)$/', trim($line), $match)) {
...
{
    load_cli_env_from_htaccess();
    $hosts = getenv('MOCK_VERIFY_ALLOWED_HOSTS') ?: 'signin.aws.amazon.com,127.0.0.1,::1';
...
{
    load_cli_env_from_htaccess();
    return getenv('MOCK_VERIFY_BUTTON_SELECTOR') ?: '[id="emailVerificationUrl"]';
}

function mock_second_button_selector()
{
    load_cli_env_from_htaccess();
    return getenv('MOCK_VERIFY_SECOND_BUTTON_SELECTOR') ?: '[id="emailVerificationUrl"]';
}
?>
